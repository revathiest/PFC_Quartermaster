...
describe('handleTradeRoute', () => {
  ...
  test('sends route embed on success', async () => {
    ...
    expect(buildBestTradesEmbed).toHaveBeenCalledWith('A', [{ totalProfit: 10 }]);
  ...
  test('warns when no profitable trades', async () => {
    const interaction = new MockInteraction({ options: { from: 'A', to: 'B' } });
    getBuyOptionsAtLocation.mockResolvedValue([{ terminal: { name: 'A' }, commodity_name: 'X', price_buy: 10, scu_buy: 1 }]);
    getSellOptionsAtLocation.mockResolvedValue([{ terminal: { name: 'B' }, commodity_name: 'X', price_sell: 5 }]);
    resolveBestMatchingTerminal.mockReturnValueOnce({ name: 'A' }).mockReturnValueOnce({ name: 'B' });
    ...
    expect(safeReply).toHaveBeenCalledWith(
      interaction,
      expect.objectContaining({ content: expect.stringContaining('No profitable trades') })
    );
  });
});
