...
module.exports = {
  ...
  async option(interaction) {
    ...
    const purchaseLocations = await db.UexVehiclePurchasePrice.findAll({
      where: { id_vehicle: vehicle.id },
      include: { model: db.UexTerminal, as: 'terminal' },
      order: [['price_buy', 'ASC']]
    });
    ...
    if (purchaseLocations.length) {
      ...
      console.warn(`[WARN] No purchase locations found for vehicle ID ${vehicle.id}`);
      embed.addFields({
        name: 'Purchase Locations',
